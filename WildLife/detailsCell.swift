...
class detailsCell: UICollectionViewCell {
    ...
    //dynamically setting label size
            if let expla = animal?.more?[i!].explanation{
                ...
                let estimatedExplanation = NSString(string: expla).boundingRect(with: size, options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize:19)] , context: nil)
            ...
            if let about = animal?.info {
                ...
                let estimatedAbout = NSString(string: about).boundingRect(with: size, options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize:16)] , context: nil)
